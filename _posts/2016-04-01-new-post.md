...
and viewing the app in our browser [http://localhost:4200](localhost):
...
As a way of recapping what we're building, let's take a look at our component
hierarchy:
...
Now we can use the `comment-box` component inside the main template for our
application:
...
Whether you're a fan of the [JSX syntax][jsx-syntax] or not, with React, you'll
be writing your HTML markup inside the JavaScript file for your component,
alongside your component's logic.

Ember uses Handlebars templates to define the presentational markup for
components. You don't need to write any JavaScript until you need to add custom
logic to your component.
...
It's time to create a component that depends on data that is passed in from its
parent context. We're going to tackle this one from the opposite direction,
starting with the invocation of the `comment-list-item`:
...
1. We're passing values for the `author` property of each component.
...
In React, data passed to a component is available inside the
component via `this.props`. Ember handles this a little bit differently, setting
data passed to a component as properties directly on the component.

We can access a component's properties from its template:
...
provided to our component.

#### `yield` vs. `this.props.children`

TODO: The React version of this component uses `this.props.children` to specify where
child elements should be rendered. The two features
...
Next, update the template for `comment-list` to convert the text of each
component from Markdown to HTML using the
[markdown-render helper][markdown-render-helper]:
