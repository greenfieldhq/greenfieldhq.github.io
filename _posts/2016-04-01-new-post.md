...
1. We're passing values for the `author` attribute of each component.
...
Note: **Make sure to restart your local server after installing the addon.**
...
- Ember generates the default `Ember.Controller` if we haven't defined one,
  which was happening until now.
- We extended the default `Ember.Controller` to provide a custom
  `ApplicationController`. This controller will now be used instead of the
  default generated instance of `Ember.Controller`.
